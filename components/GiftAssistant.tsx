...
// Fallback data for when API is completely exhausted (Rate Limit 429)
...
export const getGiftSuggestion = async (description: string): Promise<GiftSuggestionResponse> => {
  ...
  try {
    console.log("Plan A: Intentando generar imagen con gemini-2.5-flash-image...");
    ...
    // 2. PLAN B: Si falla la imagen, intentamos generar solo texto y usar imagen genérica
    console.warn("Plan A falló (posible cuota imagen). Intentando Plan B (Texto)...");
    ...
    try {
      ...
      // 3. PLAN C (ULTIMATE FALLBACK): Si todo falla (Rate Limits), usamos datos estáticos
      console.error("Plan B falló (posible cuota texto). Activando Plan C (Offline Mode).");
      
      // Seleccionar una sugerencia aleatoria de las predefinidas
      const fallback = OFFLINE_SUGGESTIONS[Math.floor(Math.random() * OFFLINE_SUGGESTIONS.length)];
      
      return {
        ...fallback,
        description: fallback.description + " (Nota: Sugerencia destacada de nuestra colección debido a alta demanda en el servidor de diseño)."
      };
    }
  }
};
